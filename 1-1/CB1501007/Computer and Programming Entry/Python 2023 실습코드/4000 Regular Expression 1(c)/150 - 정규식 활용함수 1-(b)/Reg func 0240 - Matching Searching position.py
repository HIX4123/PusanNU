...
text = "PythonX Ruby rubby Pythones cats pythoned rubbyed python. ended"
mypat = re.compile(r"python..|Python?")
m01 = re.match(mypat, text)
m02 = re.search(mypat, text)

if m01:
    print("group()", m01.group())  # 일치된 문자열 반환 group()
    print("start()", m01.start())  # 일치된 첫 위치
    print("end()", m01.end())  # 일치된 끝 위치
    print("span()", m01.span())  # 일치된 구간
