...
for w in  range(N):
    lookup_dict("countenance", KaraDict )
tend = time.process_time()
print( f'N={ N:7} dict에서 있는 원소 검사= {tend-tbegin:10.7f} sec.')

tbegin= time.process_time()
for w in  range(N):
    lookup_list("countenance" , KaraList)
tend = time.process_time()
print( f'N={ N:7} list에서 있는 원소 검사= {tend-tbegin:10.7f} sec.')
