...
def spiralize_grid(grid, column, row):
    ...
    for i in range(column * row):
        ...
        if (
            ...
            or x + dx == column  # 다음 칸의 x좌표가 column이 되는 경우 -> 오른쪽 벽에 다다른 경우
            or y + dy == row  # 다음 칸의 y좌표가 row가 되는 경우 -> 아래쪽 벽에 다다른 경우
...
# 그리드 생성
grid = spiralize_grid(np.zeros((row, column), int).tolist(), column, row)
